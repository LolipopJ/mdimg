...
const cheerio = require$$2__default["default"];
...
const puppeteer = require$$2__default$1["default"];
const {
  parseMarkdown
} = mdParser;
const {
...
async function convert2img({
  ...
  if (_body) {
    if (_encoding === "binary") {
      ...
      if (log) {
        console.log(`Convert to BASE64 encoded string successfully!${_quality ? " Iamge quality: " + _quality : ""}\n${_outputBase64String}`);
      }
